...
while (system_mode>=30)&&(system_mode<=35)
    ...
    for trial = 1:ntrials+1; %%+1 so that it repeats the first trial twice
        ...
        for frame = 1:trialduration
            ...
            if EEG
                if frame ==1
                    io64(ioObj,portcode, 30); %% trigger for trial start
                    %% 88 is trial ended
                end
            end
...
%             drawSquare;
            ...
            if (pastkeyIsDown == 1)&&(currkeyIsDown == 0)
        ...
        if system_mode ~= 1;
            ...
            dataTopLeft = [options(trialdata{trial}.condition,:) catchTrials(options(trialdata{trial}.condition,1),1:4) Struct2mat(trialdata{trial}) catchTrials(options(trialdata{trial}.condition,1),6) catchTrials(options(trialdata{trial}.condition,1),7) buttonpressTopLeft];
            dataTopRight = [options(trialdata{trial}.condition,:) catchTrials(options(trialdata{trial}.condition,1),1:4) Struct2mat(trialdata{trial}) catchTrials(options(trialdata{trial}.condition,1),6) catchTrials(options(trialdata{trial}.condition,1),7) buttonpressTopRight];
            dataBottomLeft = [options(trialdata{trial}.condition,:) catchTrials(options(trialdata{trial}.condition,1),1:4) Struct2mat(trialdata{trial}) catchTrials(options(trialdata{trial}.condition,1),6) catchTrials(options(trialdata{trial}.condition,1),7) buttonpressBottomLeft];
            dataBottomRight = [options(trialdata{trial}.condition,:) catchTrials(options(trialdata{trial}.condition,1),1:4) Struct2mat(trialdata{trial}) catchTrials(options(trialdata{trial}.condition,1),6) catchTrials(options(trialdata{trial}.condition,1),7) buttonpressBottomRight];
            ...
            filename = ['TargetsDiffFre_Trial' num2str(trial) '_data.mat'];
